...
function ByWeeks({ tasks, setTasks, setSelectedTask }) {
  ...
  let next_week_styles = [styles.changeWeekButton];
  next_week_styles.push(styles.active);
  ...
  const renderTaskBlock = (date) => {
    ...
    return (
      <div key={date.toDateString()} className={styles.items}>
        <h3 className={styles.relative_date}>
  ...
  return (
    <>
      ...
      <button
        className={next_week_styles.join(" ")}
        onClick={() => {
          setDate((prev) => {
            let newDay = new Date(prev);
            newDay.setDate(prev.getDate() + 7);
      ...
      <div className={styles.header}>
        {week.map((date) => (
          <div className={ifButtonDisabled(date, styles.header__item)}>
